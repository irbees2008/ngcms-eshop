...
<div class="row mb-2">
	<div class="col-12">
		<ol class="breadcrumb m-0">
			...
			<li class="breadcrumb-item active">{{ lang['rewrite'] }}</li>
		</ol>
		<h4>{{ lang['rewrite'] }}</h4>
	</div>
</div>
...
var flags = '<b><span style="color: ' + (
row['flagPrimary'] ? 'blue' : '#E0E0E0'
) + ';">Pri</span> ' + '<span style="color: ' + (
row['flagFailContinue'] ? 'red' : '#E0E0E0'
) + ';">FFC</span> ' + '<span style="color: ' + (
row['flagDisabled'] ? 'red' : '#E0E0E0'
) + ';">' + (
row['flagDisabled'] ? 'OFF' : 'On'
) + '</span></b>';
...
post('admin.rewrite.submit', dData, false).then(function (response) {
ngNotifySticker(NGCMS.lang['fmsg.save.done'], {closeBTN: true});
...
if ((dConfig[plugin] != null) && (dConfig[plugin][cmd] != null) && (dConfig[plugin][cmd]['descr'] != null)) { // Description
...
var tmp;
if (currentEditRow > 0) { // Reject previous edit mode
...
if (confirm('{{ lang['fmsg.edit.rowdel_confirm'] }}' + id)) { // Delete with renumbering
...
/**
	 ...
	 * @returns {void}
	 */
function reSubmitEdit() { // Check mode: add or edit
