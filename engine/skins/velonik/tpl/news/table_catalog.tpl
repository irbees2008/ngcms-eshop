	...
	<table width="1000" border="0" cellspacing="0" cellpadding="0" class="editfilter">
		<tr>
			<td valign="top" width="100%">
				&nbsp; Поиск:
				<input name="sl" type="text" class="bfsearch" size="60" value="{{ sl }}"/>
		...
		<tr>
			<td valign="top">
				<table width="250" cellspacing="0" cellpadding="0" border="0" style="margin-right: 2px;">
					...
					<tbody>
						<tr>
							<td{{cat.cutter}}</div {% if (cat_active < 1) %}style="background-color: #EEEEEE;"{% endif %}> <img alt="+" height="18" width="18" src="{{ skins_url }}/images/catmenu/plus.png"/> <a href="?mod=news">Все категории</a></td> </tr> {% for cat in catmenu %} <tr> <td {% if (cat.flags.selected) %}style="background-color: #EEEEEE;"{% endif %}> <div style="float: left; margin-right: 5px;">>
								<div style="float: left;">
									<a href="?mod=news&category={{ cat.id }}">{{ cat.name }}</a>
									{% if (cat.posts>0) %}
										[
										{{ cat.posts }}]
									{% endif %}
	...
	<table border="0" cellspacing="0" cellpadding="0" class="content" align="center">
		...
		<tbody>
			{% for entry in entries %}
				<tr align="left">
					<td width="16" class="contentEntry1" cellspacing="0" cellpadding="0" style="padding:0; margin:0;">
						{% if entry.flags.mainpage %}
							<img src="{{ skins_url }}/images/mainpage.png" border="0" width="16" height="16" title="Main"/>
						{% endif %}
					</td>
					...
					<td class="contentEntry1">
						{% if entry.flags.status %}
							<img src="{{ skins_url }}/images/yes.png" alt="{{ lang.approved }}"/>
						{% else %}
							<img src="{{ skins_url }}/images/no.png" alt="{{ lang.unapproved }}"/>
						{% endif %}
					</td>
					<td class="contentEntry1">
