<div id="tags" class="btn-toolbar mb-3" role="toolbar">
	...
	<div class="btn-group btn-group-sm mr-2">
		{% if pluginIsActive('bb_media') %}
			<button id="tags-media" type="button" class="btn btn-outline-dark" data-toggle="modal" data-target="#modal-insert-media" onclick="prepareMediaModal({{ area }})" title="{{ lang['tags.media'] }}">
				<i class="fa fa-play-circle"></i>
			</button>
		{% else %}
			<button type="button" class="btn btn-outline-dark" title="{{ lang['tags.media'] }}" onclick="try{ if(window.show_info){show_info('{{ lang['media.enable']|e('js') }}');} else { alert('{{ lang['media.enable']|e('js') }}'); } }catch(e){ alert('{{ lang['media.enable']|e('js') }}'); } return false;">
				<i class="fa fa-play-circle"></i>
			</button>
		{% endif %}
	</div>
...
<!-- Modal: Insert URL / Email / Image + JS helpers -->
