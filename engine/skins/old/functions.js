...
function check_uncheck_all(form, prefix) {
  ...
  for (var i = 0; i < elements.length; i++) {
    ...
    if (element.type == "checkbox" && element.name != "master_box") {
      if (
        prefix.length == 0 ||
        element.name.substr(0, prefix.length) == prefix
      ) {
...
// Не используем `var`, чтобы не создавать неконфигурируемое свойство на window
// Это мешает страницам, которые объявляют currentInputAreaID через Object.defineProperty
if (typeof window !== "undefined") {
  if (typeof window.currentInputAreaID === "undefined") {
    try {
      // Создаём настраиваемое свойство, если нужно
      ...
      // Fallback на простое присваивание
      window.currentInputAreaID = window.currentInputAreaID || "content";
    }
  }
}
