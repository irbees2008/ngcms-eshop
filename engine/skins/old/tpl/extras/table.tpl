...
if ((document.styleSheets[sIdx].href != null) && (document.styleSheets[sIdx].href.indexOf('/skins/default/style.css'))) { // Catched
...
if (sText == '.pluginentryactive td') 
sIndexActive = i;

if (sText == '.pluginentryinactive td') 
sIndexInactive = i;

if (sText == '.pluginentryuninstalled td') 
sIndexUninstalled = i;
...
<div id="pluginMenu">
	<table border="0" width="100%" cellpadding="0" cellspacing="0">
		<tr>
			<td width="100%" colspan="5" class="contentHead"><img src="{{ skins_url }}/images/nav.gif" hspace="8"><a href="admin.php?mod=extras">{{ lang['extras'] }}</a>
			</td>
		</tr>
	</table>
	<table border="0" width="100%" cellpadding="0" cellspacing="0">
		...
		<tbody id="entryList">
			{% for entry in entries %}
				<tr align="left" class="{{ entry.style }}" id="plugin_{{ entry.id }}">
					...
					<td>{{ entry.version }}</td>
					<td nowrap>
						<a href="{{ entry.readme }}">readme</a>
						<a href="{{ entry.history }}">История</a>
					</td>
					<td>{{ entry.description }}</td>
					<td>{{ entry.author_url }}</td>
					<td nowrap="nowrap">{{ entry.link }}
						{{ entry.install }}</td>
				</tr>
			{% endfor %}
		</tbody>
