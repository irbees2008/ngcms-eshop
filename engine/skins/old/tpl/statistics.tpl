...
<table border="0" cellspacing="0" cellpadding="0" class="content" align="center">
	<tr>
		<td width="50%" style="padding-right:10px;" valign="top">
			<table border="0" width="100%" cellspacing="0" cellpadding="0">
				...
				<tr>
					<td width="50%" class="contentEntry1">{{ lang['php_version'] }}
						/
						{{ lang['mysql_version'] }}</td>
					<td width="50%" class="contentEntry2">{{ php_version }}
						/
						{{ mysql_version }}</td>
				</tr>
				<tr>
		...
		<td width="50%" style="padding-left:10px;" valign="top">
			<table border="0" width="100%" cellspacing="0" cellpadding="0">
				...
				<tr>
					<td width="50%" class="contentEntry1">{{ lang['git_version'] }}</td>
					<td width="50%" class="contentEntry2">
						<span id="syncSVNVersion">loading..</span>
					</td>
				</tr>
				<tr>
					<td width="50%" class="contentEntry1">{{ lang['update_status'] }}</td>
					<td width="50%" class="contentEntry2">
						<span id="needUpdate">checking..</span>
					</td>
				</tr>
			</table>
		</td>
		...
		<td width="50%" style="padding-right:10px;" valign="top">
			<table border="0" width="100%" cellspacing="0" cellpadding="0">
				...
				<tr>
					...
					<td class="contentEntry1" align="right">
						&nbsp;
						{{ lang['permissions'] }}</td>
				</tr>
				<tr>
			...
			<table border="0" width="100%" cellspacing="0" cellpadding="0">
				...
				<tr>
					...
					<td width="30%" class="contentEntry2">
						<a href="?mod=news&status=1">{{ news_draft }}</a>
						/
						<a href="?mod=news&status=2">{{ news_unapp }}</a>
						/
						<a href="?mod=news&status=3">{{ news }}</a>
					</td>
				</tr>
				<tr>
				...
				<tr>
					<td width="70%" class="contentEntry1">{{ lang['cache.size'] }}</td>
					<td width="30%" class="contentEntry2">{% if cache_size %}{{ cache_size }}{% else %}Недоступно{% endif %}</td>
				</tr>
				<tr>
					<td colspan="2" class="contentEntry1">
						<form method="post" action="{{ php_self }}?mod=statistics" style="margin:0;">
							<input type="hidden" name="action" value="clearcache"/>
							<input type="hidden" name="token" value="{{ token }}"/>
							<input type="submit" class="button" value="{{ lang['cache.clean'] }}" onclick="return confirm('Очистить кэш?');"/>
						</form>
					</td>
				</tr>
			</table>
	...
	<tr>
		<td width="50%" style="padding-right:10px;" valign="top">
			<table border="0" width="100%" cellspacing="0" cellpadding="0">
				<tr>
					<td colspan="2" class="contentHead"><img src="{{ skins_url }}/images/nav.gif" hspace="8" alt=""/>{{ lang['note'] }}</td>
				</tr>
				<tr>
		...
		<td width="50%" style="padding-left:10px;" valign="top">
			{% if (flags.confError) %}
				...
				<table border="0" width="100%" cellspacing="0" cellpadding="0">
					<tr>
						<td colspan="2" class="contentHead"><img src="{{ skins_url }}/images/nav.gif" hspace="8" alt=""/><font color="red">{{ lang['pconfig.error'] }}</font>
						</td>
					</tr>
					<tr>
						<td>
...
<script type="text/javascript">
	// Функции кэша недоступны в старом шаблоне
...
<script>
	...
	setTimeout(function() {
		...
		fetch('https://api.github.com/repos/irbees2008/ngcms-core/releases/latest')
			.then(response => response.json())
			.then(data => {
				if (data.tag_name) {
					document.getElementById('syncLastVersion').innerHTML = '<a href="' + data.zipball_url + '">' + data.tag_name + '</a>';
					document.getElementById('needUpdate').innerHTML = 'Доступна версия ' + data.tag_name;
				}
			})
			.catch(() => {
				document.getElementById('syncLastVersion').innerHTML = 'Ошибка загрузки';
			});
		// Загрузка информации о коммитах
		fetch('https://api.github.com/repos/irbees2008/ngcms-core/commits')
			.then(response => response.json())
			.then(data => {
				if (data[0]) {
					document.getElementById('syncSVNVersion').innerHTML = '<a href="' + data[0].html_url + '" target="_blank">' + data[0].sha.slice(0, 7) + '</a>';
				}
			})
			.catch(() => {
				document.getElementById('syncSVNVersion').innerHTML = 'Ошибка загрузки';
			});
	}, 1000);
</script>
