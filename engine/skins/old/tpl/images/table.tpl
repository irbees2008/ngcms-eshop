...
<input type="button" value='{l_upload}' class="button" onclick="$('#fileUploadInput').uploadifive('upload');" />
</td>
</tr>
</table>
</form>
...
$(document).ready(function() {
	$('#fileUploadInput').uploadifive({
		...
		onUpload: function(filesToUpload) {
			$('#fileUploadInput').data('uploadifive').settings.formData = {
				ngAuthCookie: '{authcookie}',
				uploadType: 'image',
				category: document.getElementById('categorySelect').value,
				rand: document.getElementById('flagRand').checked ? 1 : 0,
				replace: document.getElementById('flagReplace').checked ? 1 : 0,
				thumb: document.getElementById('flagThumb').checked ? 1 : 0,
				stamp: document.getElementById('flagStamp').checked ? 1 : 0,
				shadow: document.getElementById('flagShadow').checked ? 1 : 0
			};
		},
		onUploadComplete: function(fileObj, data) {
			try {
				...
				fileObj.queueItem.find('.fileinfo')
					.replaceWith(
						response.status
						? '<div style="color: green;">' + response.errorText + '</div>'
						: '<div style="color: red;">(' + response.errorCode + ') ' + response.errorText + ' ' + response.errorDescription +'</div>'
					);
			} catch (err) {
