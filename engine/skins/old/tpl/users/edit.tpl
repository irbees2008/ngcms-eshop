		...
		<tr>
			<td width="100%" style="padding-right:10px;" valign="top">
				<table border="0" width="100%" cellspacing="0" cellpadding="0" align="left">
					<tr>
						<td width="100%" colspan="2" class="contentHead"><img src="{{ skins_url }}/images/nav.gif" hspace="8"/><a href="?mod=users">{{ lang['users_title'] }}</a>
							&#8594;
							{{ lang['profile_of'] }}
							"{{ name }}"</td>
					</tr>
					...
					<tr>
						...
						<td width="50%" class="contentEntry2" valign="middle"><input class="password" name="password" size="40" maxlength="16"/><br/><small>{{ lang['pass_left'] }}</small>
						</td>
					</tr>
					<tr>
					...
					<tr>
						...
						<td width="50%" class="contentEntry2" valign="middle">
							<textarea name="info" rows="7" cols="60">{{ info }}</textarea>
						</td>
					</tr>
					<tr>
						<td width="50%" class="contentEntry1">{{ lang['avatar'] }}</td>
						<td width="50%" class="contentEntry2" valign="middle">
							<input type="file" name="newavatar" size="40"/>
							<br/>
							<small>{{ avatar_hint }}</small>
							{% if flags.avatarAllowed and flags.hasAvatar %}
								<br/>
								<img src="{{ avatar }}" style="margin: 5px; border: 0px; max-width: 80px; max-height: 80px;" alt=""/>
								<br/>
								<input type="checkbox" name="delavatar" id="delavatar" class="check"/>&nbsp;
								<label for="delavatar">{{ lang['delete_avatar'] }}</label>
							{% endif %}
							{% if not flags.avatarAllowed %}
								<br/>
								<small>{{ lang['avatars_disabled'] }}</small>
							{% endif %}
						</td>
					</tr>
				</table>
		...
		<tr align="center">
			<td width="100%" class="contentEdit" colspan="2">
				{% if (perm.modify) %}<input type="submit" value="{{ lang['save'] }}" class="button"/>
					<input type="button" value="{{ lang['cancel'] }}" onclick="history.back();" class="button"/>
				{% endif %}&nbsp;
