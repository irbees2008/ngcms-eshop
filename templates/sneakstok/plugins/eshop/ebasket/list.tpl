...
<form class="ui form" action="{{ basket_link }}" method="post">
		...
		<tbody>
			...
			<tr class="cart_item">
				<td class="product-remove" style="vertical-align: middle">
					<a
						class="ui icon remove cart_quantity_delete"
					...
					<a href="{{ entry.xfields.item.view_link }}"
						><img
							...
							src="{% if (entry.xfields.item.image_filepath) %}{{
								home
							}}/uploads/eshop/products/{{ entry.xfields.item.id }}/thumb/{{
				...
				<td class="product-quantity">
					...
					<div class="quantity">
						<input
							...
							title="Кол-во"
							class="input-text qty text cart_quantity_input"
							size="2" />
					</div>
					<div
						class="input-group quantity-control"
				...
				<td class="product-subtotal">
					<span
						class="amount"
						>{{ (entry.price * entry.count * system_flags.eshop.currency[0].rate_from / system_flags.eshop.current_currency.rate_from)|number_format(2, '.', '') }}</span
					>
	...
	<div class="ui two column grid">
		<div class="column actions">
			{% if (formEntry.error) %} {% for error in formEntry.error %}
			<p>{{ error }}</p>
			{% endfor %} {% endif %}
			<p
				...
				<label for="billing_first_name" class=""
					>Имя <abbr class="required" title="обязательно">*</abbr></label
				><input
					type="text"
					class="input-text"
			...
			<p
				...
				id="billing_first_name_field">
				<label for="billing_first_name" class=""
					>Email <abbr class="required" title="обязательно">*</abbr></label
				><input
					type="text"
					class="input-text"
					name="userInfo[email]"
			...
			<p
				...
				id="billing_first_name_field">
				<label for="billing_first_name" class=""
					>Телефон <abbr class="required" title="обязательно">*</abbr></label
				><input
					type="text"
					class="input-text"
					name="userInfo[phone]"
			...
			<h3 class="totals-header">
				...
				<span id="total_cart"
					><strong
						><span
							class="total_amount"
							>{{ (total * system_flags.eshop.current_currency.rate_from)|number_format(2, '.', '') }}</span
						>
						<span>{{ system_flags.eshop.current_currency.sign }}</span></strong
					>
				</span>
			</h3>
			<input
				type="submit"
				class="ui button black large"
			...
			rpcEshopRequest(
				...
				function (resTX) {
					...
					click_this
						...
						.parent()
						.parent()
						.find("td[class='product-subtotal']")
						.find("span[class='amount']")
						.text(total);
