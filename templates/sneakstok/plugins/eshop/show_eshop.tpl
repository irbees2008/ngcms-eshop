...
<div id="product">
	<div class="ui grid stackable container">
		<div class="nine wide column">
			...
			<div
				...
				<div data-slidr="{{ loop.index }}" style="text-align: center">
					<img
						src="{{ home }}/uploads/eshop/products/{{ id }}/{{
							entry.filepath
						}}"
						title="{{ name }}"
						alt="" />
		...
		<div class="seven wide column" style="height: 600px">
            ...
            -->
			<div class="prices">
				{% if (not (entriesVariants[0].compare_price == '0.00')) and (not
				(entriesVariants[0].compare_price == '')) %}
				<span class="old_price"
					>{{ (entriesVariants[0].compare_price * system_flags.eshop.currency[0].rate_from / system_flags.eshop.current_currency.rate_from)|number_format(2, '.', '') }}
					{{ system_flags.eshop.current_currency.sign }})
				</span>
				{% endif %} {% if (entriesVariants[0]) %}
				<span class="current_price"
					>{{ (entriesVariants[0].price * system_flags.eshop.currency[0].rate_from / system_flags.eshop.current_currency.rate_from)|number_format(2, '.', '') }}
					{{ system_flags.eshop.current_currency.sign }}
				</span>
				{% endif %}
				<div
					class="ui large button buy radius-none orderBut"
					data-id="{{ id }}"
					data-variant="{{ entriesVariants[0]['id'] }}"
...
<!-- End Product -->
{% if (entriesRelated) %}
<div class="ui grey horizontal divider container" style="margin: 1em 0">
...
<div class="ui stackable four column grid container" id="productsPreview">
	...
	<div class="column">
		<div class="product-preview">
			...
			<a
				class="image"
				style="background:url({% if (entry.images[0].filepath) %}{{
					home
				}}/uploads/eshop/products/{{ entry.id }}/thumb/{{
					entry.images[0].filepath
				}}{% else %}{{
					home
				}}/engine/plugins/eshop/tpl/img/img_none.jpg{% endif %}) 50% 50% /cover #eee"></a>
			<a class="name">{{ entry.name }}</a>
			<div class="prices">
				{% if (entry.variants[0].price) %}<span class="old"
					...
					{{ system_flags.eshop.current_currency.sign }}</span
				>{% endif %} {% if (not (entry.variants[0].compare_price == '0.00')) and
				(not (entry.variants[0].compare_price == '')) %}<span class="current"
