<div class="block-title">
	{{ lang.uprofile["profile_of"] }} {{ user.name }} {% if
	(user.flags.isOwnProfile) %}[ <a href="/profile.html">редактировать</a> ]{%
	endif %}
</div>
<div class="block-user-info">
	<div class="avatar">
		<img src="{{ user.avatar }}" alt="" />
		{% if not (global.user.status == 0) %} {% if pluginIsActive('pm') %}<a
			href="/plugin/pm/?action=write&name={{ user.name }}"
			>написать ЛС</a
		>{% endif %} {% endif %}
	</div>
	<div class="avatar">
