...
<div class="frame-inside">
	<div class="container">
        ...
        <div class="header-category f-s_0">
			...
			<ul
				class="animateListItems items items-catalog items-product tablemini"
				id="items-catalog-main">
				<!-- Include template for one product item-->

				{% for entry in entries %}
				<li
					class="globalFrameProduct {% if (entry.variants[0].stock == 0) or (entry.variants[0].stock == 1) %}not-avail{% elseif (entry.variants[0].stock == 5) %}to-cart{% endif %}"
					data-pos="top">
					<!-- Start. Photo & Name product -->
					<a href="{{ entry.fulllink }}" class="frame-photo-title">
						<span class="photo-block">
							<span class="helper"></span>
							{% if (entry.images[0].filepath) %}<img
								src="{{ home }}/uploads/eshop/products/{{ entry.id }}/thumb/{{
									entry.images[0].filepath
								}}"
								class="vImg" />{% else %}<img
					...
					<div class="description">
						...
						<div class="frame-variant-name-code">
							<span class="frame-variant-code frameVariantCode"
								>Артикул:
								<span class="code js-code"> {{ entry.code }} </span>
							</span>
						</div>
        ...
        -->
							...
							<span class="current-prices f-s_0">
								<span class="price-new">
									<span>
										...
										<span
											class="price priceVariant"
											>{{ (entry.variants[0].price * system_flags.eshop.currency[0].rate_from / system_flags.eshop.current_currency.rate_from)|number_format(2, '.', '') }}</span
										>
										{{ system_flags.eshop.current_currency.sign }}
										{% endif %}
									</span>
								</span>
								{% if (not (entry.variants[0].compare_price == '0.00')) and (not
								(entry.variants[0].compare_price == '')) %}
								<span class="price-add">
									<span>
										<span class="price addCurrPrice"
											><s
												>{{ (entry.variants[0].compare_price * system_flags.eshop.currency[0].rate_from / system_flags.eshop.current_currency.rate_from)|number_format(2, '.', '') }}</s
											></span
										>
						...
						<div
							class="funcs-buttons frame-without-top"
							style="position: relative; top: 0px">
							<div class="no-vis-table">
								{% if (entry.variants[0].stock == 0) or (entry.variants[0].stock
								== 1) %}
								<div class="js-variant-5834 js-variant">
									<div class="alert-exists">Нет в наличии</div>
									<div class="btn-not-avail">
										<button
											class="infoBut isDrop"
											type="button"
											data-drop=".drop-report"></button>
									</div>
								</div>
						...
						<div
							class="frame-without-top"
							style="position: relative; top: 40px">
							<div class="frame-wish-compare-list no-vis-table t-a_j">
								<div class="frame-btn-comp">
									<div class="btn-compare">
										<div
											class="toCompare btnCompare {% if (entry.compare) %}active{% endif %}"
											data-id="{{ entry.id }}"
											type="button"
											data-title="Сравнить"
											...
											data-rel="tooltip">
											<span
												class="niceCheck nstcheck {% if (entry.compare) %}active{% endif %}">
												<input
													type="checkbox"
													{%
													if
						...
						<div
							...
							<div class="no-vis-table">
								...
								<div class="short-desc">
									{{entry.annotation|truncateHTML(30,'...')}}
								</div>
								<!-- End. Description-->
							</div>
						</div>
