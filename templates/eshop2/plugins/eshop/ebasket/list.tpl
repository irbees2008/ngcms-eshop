					...
					<div class="horizontal-form order-form big-title">
						<div class="inside-padd">
							...
							<div class="groups-form">
								<label>
									<span class="title">
										<span class="p_r"> Имя <span class="must">*</span> </span>
									</span>
									<span class="frame-form-field">
										<input
											type="text"
											value="{% if (formEntry.error) %}{{
												formEntry.name
											}}{% else %}{{ global.user.xfields_name }}{% endif %}"
											name="userInfo[fullName]" />
									</span>
								</label>
								<label>
									<span class="title">
										<span class="p_r"> Email <span class="must">*</span> </span>
									</span>
									<span class="frame-form-field">
										<input
											type="text"
								...
								<label>
									...
									<span class="frame-form-field">
										<input
											...
											value="{% if (formEntry.error) %}{{
												formEntry.phone
											}}{% else %}{{ global.user.xfields_phone }}{% endif %}"
											class="" />
									</span>
								</label>
							</div>
							<div class="groups-form">
								...
								<label>
									<span class="title">
										<span class="p_r"> Адрес доставки </span>
									</span>
									<span class="frame-form-field">
										<input
								...
								<div class="frame-label">
									<div class="frame-form-field">
										<button
											type="button"
											class="m-b_5 isDrop"
											data-drop=".hidden-comment"
											data-place="inherit"
											data-overlay-opacity="0">
											<span class="d_l_1">Добавить комментарий к заказу</span>
										</button>
										<div
											class="hidden-comment drop"
											data-rel=""
											data-elrun=".hidden-comment"
											style="display: none">
											<textarea name="userInfo[commentText]">{{
												formEntry.vnames.comment
											}}</textarea>
										</div>
									</div>
								</div>
								...
								<tbody>
									...
									<tr class="items items-bask cart-product items-product">
										<td class="frame-remove-bask-btn">
											<button
												type="button"
												class="icon_times_cart cart_quantity_delete"
												data-id="{{ entry.id }}"
												data-linked_ds="{{ entry.linked_ds }}"
												data-linked_id="{{ entry.linked_id }}"></button>
										</td>

										<td class="frame-items">
											<a
												class="frame-photo-title"
												href="{{ entry.xfields.item.view_link }}"
												><span class="photo-block"
													><span class="helper"></span>
													<img
														alt=""
														src="{% if (entry.xfields.item.image_filepath) %}{{
															home
														}}/uploads/eshop/products/{{
															entry.xfields.item.id
														}}/thumb/{{
															entry.xfields.item.image_filepath
														}}{% else %}{{
															home
														}}/engine/plugins/eshop/tpl/img/img_none.jpg{% endif %}"
												/></span>
												<span class="title">{{ entry.title }}</span></a
											>
											<div class="description">
												{{ entry.xfields.item.v_name }}
											</div>
										</td>
										<td>
												...
												<div class="frame-count">
													<span class="plus-minus">
														<div
															...
															<a
																rel="nofollow"
																class="input-group-addon cart_quantity_down"
																title="Уменьшить"
																>-</a
															>
															<input
																...
																data-id="{{ entry.id }}"
																data-linked_ds="{{ entry.linked_ds }}"
																data-linked_id="{{ entry.linked_id }}"
																data-price="{{ entry.price }}" />
															<a
																rel="nofollow"
																class="input-group-addon cart_quantity_up"
																title="Добавить"
																>+</a
															>
														</div>
												...
												<span class="current-prices f-s_0">
													<span class="price-new">
														<span>
															<span
																class="price"
																>{{ (entry.price* system_flags.eshop.currency[0].rate_from / system_flags.eshop.current_currency.rate_from)|number_format(2, '.', '') }}</span
															>
												...
												<span class="current-prices f-s_0">
													<span class="price-new">
														<span>
															<span
																class="price"
																>{{ (entry.sum * system_flags.eshop.currency[0].rate_from / system_flags.eshop.current_currency.rate_from)|number_format(2, '.', '') }}</span
															>
															{{ system_flags.eshop.current_currency.sign }}
														</span>
													</span>
											...
											<span class="price-new">
												<span>
													<span
														id="finalAmount"
														class="price"
														>{{ (total * system_flags.eshop.currency[0].rate_from / system_flags.eshop.current_currency.rate_from)|number_format(2, '.', '') }}</span
													>
													{{ system_flags.eshop.current_currency.sign }}
												</span>
											</span>
			...
			rpcEshopRequest(
				...
				function (resTX) {
					...
					click_this
						...
						.parent()
						.parent()
						.find("td[class='frame-cur-sum-price frame-sum']")
...
<style>
	.quantity-control .cart_quantity_up,
	.quantity-control .cart_quantity_down {
